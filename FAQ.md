...
CAP-Core is reserved for future work.
...
## Why is CAP-Core reserved?

CAP-Core would need to address artifact graphs, capability/runtime binding, resource and service binding, policy decisions, run lifecycle, RunEvidence, provenance, and external standard bindings. That is a larger problem and should not be smuggled into CAP-Digest.

## What should implementers build first?

Start with one source type and one fixture. The current target is `fixtures/basic-table/` plus the minimal Python reference implementation under `reference/python/`.
